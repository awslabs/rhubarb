...
MODEL_MAPPING = {
    "claude-opus": LanguageModels.CLAUDE_OPUS_4_6,
    "claude-sonnet": LanguageModels.CLAUDE_SONNET_4_6,
    "claude-haiku": LanguageModels.CLAUDE_HAIKU_4_5,
    "nova-pro": LanguageModels.NOVA_PRO,
    "nova-lite": LanguageModels.NOVA_LITE,
    "nova-2-lite": LanguageModels.NOVA_2_LITE,
}
...
class RhubarbMCPServer:
    ...
    def _setup_tools(self):
        ...
        def analyze_document(
            ...
            try:
                if model not in MODEL_MAPPING:
                    return {
                        "error": f"Invalid model: {model}. Available models: {list(MODEL_MAPPING.keys())}"
                    }
                ...
                da = DocAnalysis(
                    ...
                    cross_region_inference=os.getenv("RHUBARB_CROSS_REGION", "us") or None,
                )
                ...
                for entity_name in entities:
                    if hasattr(entities_class, entity_name.lower()):
                        ...
                        custom_entity = type(
                            "Entity",
                            (),
                            {"name": entity_name, "description": f"Custom entity: {entity_name}"},
                        )()
                        entity_objects.append(custom_entity)

                result = da.run_entity(
                    message="Extract all entities from this document", entities=entity_objects
                )

                return {"success": True, "result": result, "entities_requested": entities}
        ...
        def generate_extraction_schema(
            file_path: str, extraction_description: str, assistive_rephrase: bool = False
        ) -> Dict[str, Any]:
            """
            ...
            try:
                ...
                result = da.generate_schema(
                    message=extraction_description, assistive_rephrase=assistive_rephrase
                )

                return {"success": True, "result": result, "description": extraction_description}
        ...
        def create_classification_samples(
            manifest_path: str, bucket_name: str, update_sample_id: Optional[str] = None
        ) -> Dict[str, Any]:
            """
            ...
            try:
                session = self._get_boto3_session()
                dc = DocClassification(bucket_name=bucket_name, boto3_session=session)

                result = dc.run_sampling(
                    manifest_path=manifest_path, update_sample_id=update_sample_id
                )
        ...
        @self.mcp.tool()
        def view_classification_sample(sample_id: str, bucket_name: str) -> Dict[str, Any]:
            """
        ...
        def analyze_video(
            ...
            try:
                ...
                if not file_path.startswith("s3://"):
                    return {
                        "error": "Video analysis requires S3 storage. File must be uploaded to S3."
                    }
    ...
    def _setup_resources(self):
        ...
        def get_classification_sample(bucket: str, sample_id: str) -> str:
            ...
            try:
                session = self._get_boto3_session()
                dc = DocClassification(bucket_name=bucket, boto3_session=session)
